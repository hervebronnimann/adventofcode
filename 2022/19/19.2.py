...
def qappend(dr,ds,q,qm,s):
  ...
  if t1 in dr: # we could also try every t in dr that dominates t1, but that runtime explodes...
    for s2 in dr[t1]:
...
def geode(b,T):
  ...
  b35=max(b[3],b[5])  # max cost of ore, can't spend more ore than that per minute
  ...
  while n < len(q):
    ...
    if s[2] >= b[4]: s[2]=b[4]
    if s[3] >= b[6]: s[3]=b[6]
    # also no need to have too many resources if you can't spend them
