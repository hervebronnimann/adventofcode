...
def value(c): return 1+ord(c)-ord('a') if c>='a' else 27+ord(c)-ord('A')

def common(x): return x[0].intersection(*x).pop()

groups=[ [set(x) for x in lines[n:n+3]] for n in range(0,len(lines),3) ]

print(sum([value(common(g)) for g in groups]))
