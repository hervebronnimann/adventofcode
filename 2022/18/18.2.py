...
grid=set([tuple(t) for t in product(range(X[0]+1),range(X[1]+1),range(X[2]+1)) if t not in lines])
...
dX = [[set([tuple(t[:k]+t[k+1:]) for t in shell if t[k]==x]) for x in range(X[k]+1)] for k in range(3)]
