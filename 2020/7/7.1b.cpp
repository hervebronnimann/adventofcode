...
int main(int argc, const char **argv)
{
  ...
  for (auto& x : graph) { compute(x.first, w); }
  int n =  0;
  for (auto& x : graph) { if (w[x.first]>0 && x.first!="shiny gold") ++n; }
  std::cout << "Number of bags containing a shiny gold bag: " << n << std::endl;
  return 0;
}
