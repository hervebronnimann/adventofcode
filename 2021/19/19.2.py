...
def dist1(p,q): return abs(p[0]-q[0])+abs(p[1]-q[1])+abs(p[2]-q[2])
...
''' Compute position of each scanner wrt to scanner 0, in orientation 0. '''
''' Same as part 1 except pretend that each scanner has only one point: itself, at (0,0,0). '''
seq.pop(0); s=[set([(0,0,0)]) for i in range(len(scanner))]
for j,i in seq[::-1]:
  for p in s[j]:
    dp,orient = pos2[(i,j)]
