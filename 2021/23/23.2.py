...
def hboard(board):
  ...
  return tuple([board[i] if i in board.keys() else 0 for i in range(0,50)])
...
def print_board(prefix,b,cost):
  ...
  print('%s  #%s#%s#%s#%s#  ' % (prefix,c[b[33]],c[b[35]],c[b[37]],c[b[39]]))
  print('%s  #%s#%s#%s#%s#  ' % (prefix,c[b[43]],c[b[45]],c[b[47]],c[b[49]]))
  print('%s  #########  ' % prefix)
...
iboard = hboard({
  ...
  # 23:1000, 25:100,27:10,  29:1,
  # 33:1000, 35:10, 37:1,   39:100,
  # 43:1,  45:1000, 47:100, 49:1
  13:1000, 15:1,   17:1,  19:1000,    # For my input
  23:1000, 25:100, 27:10, 29:1,
  33:1000, 35:10,  37:1,  39:100,
  43:100,  45:100, 47:10, 49:10
})
...
eboard = hboard({
  ...
  33:1, 35:10, 37:100, 39:1000,
  43:1, 45:10, 47:100, 49:1000
})
...
def neighbors(board):
  ...
  for h in hall:
    ...
    if board[d+10]!=0 and board[d+10]!=board[h]: continue # below is occupied by misplaced piece
    if board[d+20]!=0 and board[d+20]!=board[h]: continue # below is occupied by misplaced piece
    if board[d+30]!=0 and board[d+30]!=board[h]: continue # bottom is occupied by misplaced piece
    if h<d-10:
    ...
    if board[d+10]==0: d += 10; cost += 1   # slide into below cell
    if board[d+10]==0: d += 10; cost += 1   # slide into below cell
  ...
  for d in topdest:
    ...
    while d < 40 and board[d]==0: d += 10; cost += 1 # first piece in cell
    if board[d]==0: continue
    pieces={board[d2],board[d2+10],board[d2+20],board[d2+30]};
    if 0 in pieces: pieces.remove(0)
    if len(pieces)==0: continue  # all empty
    if edest[board[d]]==d2 and len(pieces)==1: continue # in its place and all identical below
