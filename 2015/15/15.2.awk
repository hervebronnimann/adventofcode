...
END {
  ...
  for (i3=0; i1+i2+i3<=tspoons; ++i3) {
    i4 = tspoons-i1-i2-i3;
    ccap=0; ddur=0; ffla=0; ttex=0; ccal=0; j=0;
    for (i in ingredient) {
       ++j; ts[i] = j==1?i1:j==2?i2:j==3?i3:i4;
       ccap += cap[i] * ts[i]; ddur += dur[i] * ts[i]; ffla += fla[i] * ts[i]; ttex += tex[i] * ts[i]; ccal += cal[i] * ts[i];
    }
    ...
    if (ccal==500 && score>hscore) hscore = score;
  }
  print hscore;
}
